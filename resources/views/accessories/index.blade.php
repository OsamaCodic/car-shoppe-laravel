...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row mb-2">
            <div class="col-md-10">
                <h1 class="h3 mb-2 text-gray-800">All Accessories</h1>
            </div>
            <div class="col-md-2">
                <a href="{{url('admin/accessories/create')}}" type="button" class="btn btn-block btn-primary rounded-pill btn-sm zoomBtn"><i class="fa fa-plus ml-2" aria-hidden="true"></i></a>
            </div>  
        </div>
        ...
        <div class="card shadow mb-4">
                    ...
                    <div class="col-md-5">
                        <form action="{{url('admin/search_accessory')}}" method="get" role="search" class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
                            ...
                            <div class="input-group">
                                <input type="text" name="title" class="form-control bg-white border-0 small" placeholder="Search by name" aria-label="Search" aria-describedby="basic-addon2">
                                
                                <div class="input-group-append">
                                    <button class="btn btn-primary" type="submit"><i class="fas fa-search fa-sm"></i></button>
                                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">Advance</button>
                                    <a href="{{url('admin/accessories')}}" class="btn btn-danger" type="button"><i class="fas fa-times fa-sm"></i></a>
                                </div>
                            </div>
                        </form>
                        @include('accessories.partials.search_modal')
                    </div>
                </div>
            </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                            ...
                            <tr>
                                ...
                                <th>
                                    ...
                                    @if (@$_GET['sortbyName'] == "ASC")
                                        <a href="{{url('admin/accessories?sortbyName=DESC')}}"><i class="fa fa-sort-amount-desc" aria-hidden="true"></i></a>
                                        @else
                                        <a href="{{url('admin/accessories?sortbyName=ASC')}}"><i class="fa fa-sort-amount-asc" aria-hidden="true"></i></a>
                                    @endif
                                </th>
                                <th>Images</th>
                                <th>Actions</th>
                            ...
                            @foreach ($accessories as $accessory)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ url('admin/accessories/'.$accessory->id) }}"  target="_blank" ><i class="fa fa-eye ml-2 zoom" aria-hidden="true" style="color: #7d9eff" data-toggle="tooltip" data-placement="top" title="Details"></i></a>
                                        <input class="form-check-input zoom" type="checkbox" id="delete_rows" name="delete_rows[]" value="{{$accessory->id}}" style="display:none; margin-top: 6px; margin-left: 13px;">
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    ...
                    @if ($accessories->count() == 0)
                        <p class="text-danger text-center">No accessories founds...</p>
                    @endif
                ...
                @if ($accessories->count() > 0)
                    {{-- {{ $users->links() }} --}}
                    <p>Showing {!! $accessories->firstItem() !!} to {!! $accessories->lastItem() !!}</p>
                    {{ $accessories->links() }}
                @endif
            </div>
        </div>
...
@section('javascript')
    @include('accessories.partials.js')
@endsection