...
<body class="body-wrapper">
    ...
    <section class=" bg-gray py-5">
        <div class="container">
            <form id="sellerForm" action="{{url('front/store_part_buyer_details')}}" method="POST">

                @csrf
                <h3><strong class="text-success">{{$accessory->title}}</strong> Buyer detail</h3> <hr>

                <input type="hidden" name="part_id" value="{{$accessory->id}}">
                <input type="hidden" name="user_id" value="{{Auth::user()->id}}">
                
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            <input type="text" class="form-control" name="first_name" readonly value="{{$loggedUser->first_name}}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <input type="text" class="form-control" name="last_name" readonly value="{{$loggedUser->last_name}}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <input type="text" class="form-control" name="email" readonly value="{{$loggedUser->email}}">
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                ...
                <button type="submit" id="submitBtn" class="btn btn-success rounded-pill themeBtn zoomBtn"  style="">BUY</button>
            </form>
        </div>
    </section>
    {{-- /Content --}}
